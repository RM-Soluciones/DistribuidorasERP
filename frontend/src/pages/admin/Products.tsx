...
export default function AdminProducts() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingProduct) {
      updateProduct({ id: editingProduct.id, data: payload }, {
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["products"] }); toast({ title: "Producto actualizado" }); setIsDialogOpen(false); }
      });
    } else {
      createProduct(payload, {
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["products"] }); toast({ title: "Producto creado" }); setIsDialogOpen(false); }
      });
    }
  };
  ...
  const handleDelete = (id: number) => {
    if (confirm("¿Eliminar este producto?")) {
      deleteProduct({ id }, {
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["products"] }); toast({ title: "Producto eliminado" }); }
      });
    }
  };
